  ...
  return (
    <Link href={`/agreements/${agreement.id}`} className="group block">
      <div className="glow-card glow-card-hover rounded-2xl p-5 relative overflow-hidden flex flex-col justify-between h-full">
        <div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs font-semibold px-2 py-0.5 rounded bg-slate-800 text-slate-300 border border-slate-700/60">
                LEASE #{agreement.id}
              </span>
            </div>
            <span
          ...
          {/* Financials Row */}
          <div className="grid grid-cols-2 gap-3 mb-4 rounded-xl bg-slate-900/70 p-3 border border-slate-800/80">
            <div>
              ...
              <span className="text-base font-bold text-slate-100 flex items-center gap-1">
                {agreement.rentAmount} <span className="text-xs text-indigo-400 font-mono">ETH</span>
              </span>
            </div>
            <div>
              <span className="text-[11px] text-slate-400 block font-mono">Escrow Deposit</span>
              <span className="text-base font-bold text-slate-100 flex items-center gap-1">
                {agreement.securityDeposit} <span className="text-xs text-indigo-400 font-mono">ETH</span>
              </span>
            </div>
          </div>
        ...
        {/* Action Link Footer */}
        <div className="mt-5 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs font-medium text-indigo-400 group-hover:text-indigo-300 transition-colors">
